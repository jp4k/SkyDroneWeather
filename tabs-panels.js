(() => {
  ...
  const FALLBACK_LOCATION = { lat: -30.3364, lon: -54.3200, timezone: 'America/Sao_Paulo', name: 'São Gabriel - RS' };
  const DEFAULT_REFRESH_SECONDS = 300;
  const CORE_PROVIDER_TIMEOUT_MS = 5000;
  const OPEN_METEO_TIMEOUT_MS = 8000;
  const MAX_STALE_WEATHER_CACHE_MS = 60 * 60 * 1000;
  ...
  const STATION_HEALTH_CACHE_TTL_MS = 30 * 60 * 1000;
  const STATION_WORKING_CACHE_TTL_MS = 8 * 60 * 1000;
  const STATION_OBSERVATION_MAX_AGE_MS = 6 * 60 * 60 * 1000;
  const STATION_HIGH_LATENCY_MS = 1600;
  const METAR_REPORT_CACHE_TTL_MS = 5 * 60 * 1000;
  ...
  const PROVIDER_ORDER = ['openMeteo', 'inmet', 'metarNoaa', 'noaa', 'ecmwf', 'gfs', 'icon', 'weatherApi', 'meteostat', 'demoWeather'];
  const MULTI_MODEL_PROVIDER_KEYS = ['openMeteo', 'gfs', 'icon', 'ecmwf', 'weatherApi'];
  ...
  const LOW_CONFIDENCE_RETRY_MS = 60 * 1000;
  ...
  const MODEL_DIVERGENCE_THRESHOLDS = Object.freeze({
    temperature: { medium: 3, high: 7 },
    windSpeed: { medium: 8, high: 18 },
    rainProbability: { medium: 35, high: 70 }
  });
  ...
  const PROVIDERS = [
    ...
    },
    {
      ...
      type: 'station',
      reliability: 0.94,
      availableForLocation: isLikelyBrazilCoverage,
      fetcher: fetchInmet
    },
    {
    ...
    },
    {
      ...
      type: 'official',
      reliability: 0.91,
      availableForLocation: isLikelyNoaaCoverage,
      fetcher: fetchNoaa
    },
    {
      providerKey: 'gfs',
  ...
  function isProviderEnabled(provider, location = null) {
    if (!provider?.requiresBackend) return true;
    if (!window.SkyDroneServices?.weatherService?.hasServerlessBackend?.()) {
      return false;
    }
    if (typeof provider.availableForLocation === 'function' && location) {
      return Boolean(provider.availableForLocation(location));
    }
    return true;
  }

  function getEnabledProviders(location = null) {
    return PROVIDERS.filter((provider) => {
      if (!isProviderEnabled(provider, location)) return false;
      if (typeof provider.availableForLocation === 'function' && location) {
        return Boolean(provider.availableForLocation(location));
      }
      return true;
  ...
  let specialMapOverlay = null;

  document.addEventListener('DOMContentLoaded', init);
  window.switchTab = switchTab;
  window.findNearestWeatherStation = findNearestWeatherStation;
  window.findNearestWorkingStation = findNearestWorkingStation;
  window.getCityWeather = getCityWeather;

  function init() {
    cacheDom();
    reportMissingClimateElements();
    prepareClimateShell();
    bindUI();
    ...
    if (typeof L !== 'undefined' && document.getElementById('map')) {
      ...
      console.warn('[clima] Leaflet indisponivel. O mapa da aba Clima nao sera iniciado.');
    }
  ...
  function reportMissingClimateElements() {
    [
      'temp',
      'feels',
      'vento',
      ...
      if (!document.getElementById(id)) {
        console.warn(`[clima] Elemento ausente: #${id}`);
      }
    });
  }

  function prepareClimateShell() {
    const climaRoot = document.getElementById('clima');
    if (climaRoot) {
      climaRoot.classList.add('weather-theme-initializing');
    }

    if (dom.mapModelSelect) {
      dom.mapModelSelect.innerHTML = `
        <option value="fused" selected>Modo: Open-Meteo</option>
        <option value="ecmwf">Campo: ECMWF sem chave</option>
        <option value="gfs">Campo: GFS sem chave</option>
      `;
    }

    if (dom.locationSearchInput && !dom.locationSearchInput.placeholder.includes('coordenadas')) {
      dom.locationSearchInput.placeholder = 'Buscar cidade, aeroporto, regiao ou coordenadas';
    }
  }
  ...
  function hydrateSharedWeatherCache() {
    ...
    const allowedProviders = new Set(getEnabledProviders(cachedBundle.location || state.location).map((provider) => provider.providerKey));
    ...
    setResilienceNotice(buildResilienceNotice('Usando \u00faltimo dado valido salvo'));
    setWeatherUpdateStatus('Usando \u00faltimo dado valido salvo', 'cache');
  ...
  function applyReliabilityGuard(bundle, providerRuns, cacheInfo) {
    return window.SkyDroneServices?.reliabilityService?.applyToBundle?.(bundle, providerRuns, cacheInfo) || bundle;
  }

  function saveLastValidWeatherBundle(bundle, providerRuns, cacheInfo) {
    if (!bundle?.current || cacheInfo?.used) return;
    window.SkyDroneServices?.cacheService?.saveLastValid?.(null, bundle, {
  ...
  function buildCachedWeatherResult(bundle, providerRuns, cacheInfo) {
    const adjustedBundle = applyReliabilityGuard(bundle, providerRuns, cacheInfo);
    return {
      bundle: adjustedBundle,
      providerRuns,
      cacheInfo
    };
  }

  function setWeatherUpdateStatus(message, tone = '') {
    if (dom.locationSearchStatus) {
      dom.locationSearchStatus.textContent = message;
    }
    window.SkyDroneServices?.autoUpdateService?.setStatus?.(tone || 'info', message);
  }

  function getActiveWeatherModelLabels(providerRuns = state.providerRuns) {
    return (Array.isArray(providerRuns) ? providerRuns : [])
      .filter((run) => isDisplayableLiveProvider(run))
  ...
  function isDisplayableLiveProvider(run) {
    if (!run || run.success !== true || run.status === 'loading' || run.hidden) return false;
    if (!hasUsableProviderSignal(run)) return false;
    const message = String(run.message || '').toLowerCase();
    const fromFallbackCache = message.includes('cache') || message.includes('fallback automatico') || message.includes('fallback autom');
    return !fromFallbackCache || run.status === 'online';
  }

  function getModelsSummary(providerRuns = state.providerRuns) {
    const labels = getActiveWeatherModelLabels(providerRuns);
    if (!labels.length) return 'Modelos: indisponiveis';
    const visible = labels.slice(0, 4).join(' + ');
    return labels.length > 4 ? `Modelos: ${visible} +${labels.length - 4}` : `Modelos: ${visible}`;
  }

  function getUpdateAgeText(date = state.lastUpdate) {
    if (!date) return 'Atualizacao pendente';
    const ageMs = Math.max(0, Date.now() - new Date(date).getTime());
    return `Atualizado ha ${formatDurationShort(ageMs)}`;
  }
  ...
  function rememberSuccessfulWeatherState(bundle, providerRuns, cacheInfo) {
    ...
    saveLastValidWeatherBundle(bundle, providerRuns, cacheInfo);
  }
  ...
  function restoreLastGoodWeatherState(error) {
    ...
    state.cacheInfo = {
      ...
      reason: 'Usando ultimo dado valido salvo'
    };
    state.weatherBundle = applyReliabilityGuard(state.resilience.lastGoodWeatherBundle, state.providerRuns, state.cacheInfo);
  ...
  function cacheDom() {
    [
      'loadingOverlay',
      'climaLocationLabel',
      'climateHeaderLocation',
      'climateMobileLocation',
      'climateSidebarRefresh',
      'droneSidebarRefresh',
      'currentWeatherIcon',
      'temp',
      'feels',
      ...
      'confidenceNote',
      'confidenceChip',
      'mobileConfidenceFill',
      'reliabilityDetailsBtn',
      'reliabilityModal',
      'reliabilityModalClose',
      'reliabilityModalSummary',
      'reliabilitySourcesList',
      'divergenceValue',
      'divergenceNote',
      'trendValue',
      'trendNote',
  ...
  function bindUI() {
    ...
    dom.locationSearchForm?.addEventListener('submit', async (event) => {
      event.preventDefault();
      try {
        await getCityWeather(dom.locationSearchInput?.value || '', {
          showLoading: true,
          force: true
        });
      } catch (error) {
        console.warn('Falha ao buscar clima da cidade:', error);
      }
    });
    ...
    dom.reliabilityDetailsBtn?.addEventListener('click', openReliabilityModal);
    dom.reliabilityModalClose?.addEventListener('click', closeReliabilityModal);
    dom.reliabilityModal?.addEventListener('click', (event) => {
      if (event.target === dom.reliabilityModal) {
        closeReliabilityModal();
      }
    });

    document.addEventListener('keydown', (event) => {
      if (event.key === 'Escape' && dom.reliabilityModal && !dom.reliabilityModal.hidden) {
        closeReliabilityModal();
      }
    });
    ...
    dom.mapModelSelect?.addEventListener('change', async () => {
      ...
      if (state.weatherLayer !== 'radar') {
        updateRadarStatus('Camada meteorologica atualizada.');
        await renderMapOverlays();
      }
    });
  ...
  function switchTab(tab) {
    ...
    window.VentoUXEnhancements?.handleTabChange?.(tab);
  ...
  async function refreshWeather({ showLoading = false, force = false } = {}) {
    ...
    try {
      if (showLoading) {
        showLoadingOverlay(true, 'Sincronizando dados meteorologicos...');
      }

      setWeatherUpdateStatus('Atualizando...', 'updating');
      ...
      setWeatherUpdateStatus(
        state.cacheInfo.used
          ? (state.cacheInfo.saved || state.cacheInfo.stale ? 'Usando \u00faltimo dado valido salvo' : 'Usando cache salvo')
          : 'Dados atualizados',
        state.cacheInfo.used ? 'cache' : 'success'
      );
      if (requestId === state.requestId) {
        state.resilience.lastRefreshErrorAt = 0;
        const confidence = Number(state.weatherBundle?.analytics?.confidence);
        startAutoRefresh(Number.isFinite(confidence) && confidence < 70
          ? LOW_CONFIDENCE_RETRY_MS
          : getBaseRefreshDelayMs());
      }
      ...
      setWeatherUpdateStatus('Erro na atualiza\u00e7\u00e3o', 'error');
      showError('Dados meteorologicos temporariamente indisponiveis. O painel tentara sincronizar novamente em breve.');
      if (requestId === state.requestId) {
        startAutoRefresh(getRecoveryRefreshDelayMs());
      }
    } finally {
  ...
  async function getWeatherBundleForLocation(location, { force = false, onProgress = null } = {}) {
    ...
    if (!force && cached && cacheAge <= CACHE_TTL.weather) {
      return buildCachedWeatherResult(cached.value.bundle, cached.value.providerRuns, {
        used: true,
        stale: false,
        ageMs: cacheAge,
        reason: 'Usando cache salvo'
      });
    }
    ...
    try {
      ...
      if (activeProviderCount < 1 && cached && cacheAge <= MAX_STALE_WEATHER_CACHE_MS) {
        return buildCachedWeatherResult(cached.value.bundle, cached.value.providerRuns, {
          used: true,
          stale: true,
          apiFailed: true,
      ...
      const guardedLive = {
        bundle: applyReliabilityGuard(live.bundle, live.providerRuns, liveCacheInfo),
        providerRuns: live.providerRuns
      };
      writeCache(cacheKey, guardedLive);
      saveLastValidWeatherBundle(guardedLive.bundle, guardedLive.providerRuns, liveCacheInfo);
      return {
        ...guardedLive,
        cacheInfo: liveCacheInfo
      };
    } catch (error) {
      if (cached && cacheAge <= MAX_STALE_WEATHER_CACHE_MS) {
        return buildCachedWeatherResult(cached.value.bundle, cached.value.providerRuns, {
          used: true,
          stale: true,
          apiFailed: true,
      ...
      const saved = window.SkyDroneServices?.cacheService?.loadLastValid?.(location);
      if (saved && !saved.locationMismatch) {
        const savedBundle = window.SkyDroneServices.cacheService.buildBundleFromCachedEntry(saved, location);
        const savedProviders = savedBundle?.providers || [];
        return buildCachedWeatherResult(savedBundle, savedProviders, {
          used: true,
          stale: true,
          saved: true,
          apiFailed: true,
  ...
  async function fetchLiveWeatherBundle(location, { onProgress = null } = {}) {
    ...
    const activeProviders = getEnabledProviders(location);
    const progressRuns = activeProviders.map((provider) => createLoadingProviderRun(provider));
    emitProviderProgress(progressRuns, onProgress);
    const settledRuns = await Promise.allSettled(
      activeProviders.map((provider, index) => runProvider(provider, location, context)
        .then((run) => {
    ...
    const providerRuns = settledRuns.map((result, index) => (
      result.status === 'fulfilled'
        ? result.value
        : createProviderFailureRun(activeProviders[index], result.reason, performance.now())
    ));
  ...
  function createProviderContext(location) {
    ...
    return {
      ...
      async getWorkingStationSelection() {
        if (!cache.workingStationSelectionPromise) {
          cache.workingStationSelectionPromise = findNearestWorkingStation(location.lat, location.lon, {
            name: location.name
          });
        }
        return cache.workingStationSelectionPromise;
      }
    };
  }
  ...
  async function runProvider(provider, location, context) {
    ...
    try {
      ...
      if (normalized.latencyMs > STATION_HIGH_LATENCY_MS) {
        normalized.status = 'partial';
        normalized.apiStatus = 'degraded-latency';
        normalized.failureReason = 'latencia alta';
        normalized.message = `${normalized.message} Latencia alta detectada (${normalized.latencyMs}ms); peso reduzido automaticamente.`;
        normalized.weightMultiplier = Math.min(valueOr(normalized.weightMultiplier, 1), 0.72);
        normalized.weight = computeProviderWeight(normalized);
      }
  ...
  async function fetchOpenMeteoModel(location, options) {
    ...
    const params = new URLSearchParams({
      latitude: String(toFiniteNumber(location?.lat, FALLBACK_LOCATION.lat)),
      longitude: String(toFiniteNumber(location?.lon, FALLBACK_LOCATION.lon)),
      timezone: location?.timezone || FALLBACK_LOCATION.timezone,
    ...
    const data = await fetchJson(`${OPEN_METEO_FORECAST_URL}?${params.toString()}`, { timeoutMs: OPEN_METEO_TIMEOUT_MS });
  ...
  async function fetchDemoWeather(location) {
    const start = new Date();
    start.setMinutes(0, 0, 0);

    const locationSeed = Math.abs(Math.sin((Number(location.lat) || 0) * 12.9898 + (Number(location.lon) || 0) * 78.233));
    const hourly = Array.from({ length: 7 * 24 }, (_, index) => {
      ...
      const daylightCurve = Math.sin(((hour - 8) / 24) * Math.PI * 2);
      const mildVariation = Math.sin((index + locationSeed * 8) / 5) * 1.4;
      const temperature = index === 0
        ? 25
        : roundTo(25 + daylightCurve * 3.8 + mildVariation - dayOffset * 0.15, 1);
      const rainPulse = Math.max(0, Math.sin((index - 18) / 9));
      const rainProbability = index === 0 ? 8 : Math.round(clamp(8 + rainPulse * 24 + dayOffset * 1.5, 4, 42));
      const cloudCover = index === 0 ? 18 : Math.round(clamp(18 + rainPulse * 34 + Math.max(0, -daylightCurve) * 12, 8, 72));
      const weatherCode = rainProbability >= 35 ? 61 : cloudCover <= 25 ? 0 : cloudCover >= 60 ? 3 : 2;
      const isDay = hour >= 6 && hour <= 18;
      const presentation = getWeatherPresentation(weatherCode, isDay);

      return {
        time: time.toISOString(),
        dateKey: extractDateKey(time.toISOString()),
        temperature,
        feelsLike: roundTo(temperature + (isDay ? 1 : -0.3), 1),
        humidity: Math.round(clamp(58 + rainPulse * 18 + Math.max(0, -daylightCurve) * 10, 42, 84)),
        pressure: roundTo(1014 + Math.cos(index / 12) * 2.5, 1),
        visibilityKm: roundTo(clamp(16 - rainPulse * 5 - cloudCover / 28, 7, 18), 1),
        windSpeed: roundTo(clamp(12 + Math.sin(index / 4) * 4 + dayOffset * 0.4, 6, 22), 1),
        windDirection: Math.round((85 + index * 7 + locationSeed * 40) % 360),
        windGusts: roundTo(clamp(18 + Math.sin(index / 3) * 5 + rainPulse * 6, 12, 34), 1),
        cloudCover,
        rainProbability,
        precipitation: rainProbability >= 35 ? roundTo(0.4 + rainPulse * 0.8, 1) : 0,
        uvIndex: isDay ? roundTo(clamp(7.5 - cloudCover / 25, 1.2, 8.4), 1) : 0,
        weatherCode,
        isDay,
        icon: presentation.icon,
        description: index === 0 ? 'Ceu limpo em modo demo' : presentation.label
      };
    });
    ...
    return {
      ...
      message: 'Demo automatico ativo com dados climaticos simulados, sem chave de API.',
      current: hourly[0],
      hourly,
      daily
    };
  ...
  async function fetchWeatherApi(location) {
    if (!window.SkyDroneServices?.weatherService?.hasServerlessBackend?.()) {
      throw createError('unsupported', 'WeatherAPI opcional requer backend/serverless. No GitHub Pages o painel usa apenas Open-Meteo.');
    }

    try {
      const data = await window.SkyDroneServices.weatherService.fetchWeatherApiViaBackend(location, {
        timeoutMs: CORE_PROVIDER_TIMEOUT_MS,
        days: 3
      });
      console.info('[clima] WeatherAPI carregado com sucesso.', {
        lat: location.lat,
        lon: location.lon
      });
      return normalizeWeatherApiResponse(data);
    } catch (error) {
      console.warn('[clima] Falha ao consultar WeatherAPI.', error);
      throw error;
    }
  }
  ...
  async function fetchMetarNoaa(location, provider, context) {
    ...
    const candidatePool = rankStationCandidatesByHealth(baseSelection.candidates, 'metarNoaa')
      .slice(0, METAR_STATION_CANDIDATE_LIMIT);
    if (!candidatePool.length) {
      const error = createError('unsupported', 'METAR/NOAA sem aeroporto funcional recente nas proximidades.');
      error.stationSelection = normalizeStationSelection({
        ...baseSelection,
    ...
    const reports = await fetchMetarReports(candidatePool.map((station) => station.id), {
      allowCachedFallback: true
    });
    const payloadEntries = candidatePool
      .map((station) => {
        const report = reports.get(station.id);
        if (!report) return null;

        try {
          const payload = normalizeMetarStationResponse(report, station);
          const latencyMs = Number.isFinite(reports.latencyMs) ? reports.latencyMs : null;
          const lastUpdatedAt = normalizeTimeValue(payload.current?.time) || null;
          const apiStatus = reports.cacheStatus === 'cache'
            ? 'cache-fallback'
            : Number.isFinite(latencyMs) && latencyMs > STATION_HIGH_LATENCY_MS
              ? 'degraded-latency'
              : 'online';
          const confidence = calculateStationConfidence({
            station,
            sourceKey: 'metarNoaa',
          ...
          markStationHealth('metarNoaa', enrichedStation, {
            status: apiStatus,
            latencyMs,
            lastUpdatedAt,
            confidence,
          ...
          markStationHealth('metarNoaa', station, {
            status: 'offline-controlled',
            latencyMs: reports.latencyMs,
            failureReason: getFailureReason(error)
          });
          return null;
        }
    ...
    if (!payloadEntries.length) {
      const error = createError('unsupported', 'METAR/NOAA sem observacoes recentes nos aeroportos mais proximos.');
      error.stationSelection = normalizeStationSelection({
        ...baseSelection,
        apiStatus: 'offline-controlled',
        failureReason: getFailureReason(error),
        stations: candidatePool.slice(0, 3)
      });
      throw error;
    }

    const rebuiltSelection = rebuildStationSelectionFromCandidates(baseSelection, payloadEntries.map((entry) => entry.station));
    const selection = rebuiltSelection
      ? {
          ...rebuiltSelection,
          apiStatus: payloadEntries.some((entry) => entry.payload?.stationDiagnostics?.apiStatus === 'degraded-latency') ? 'degraded-latency' : 'online',
          latencyMs: Math.round(meanOfValues(payloadEntries.map((entry) => entry.payload?.stationDiagnostics?.latencyMs)) || reports.latencyMs || 0),
          lastUpdatedAt: payloadEntries
            .map((entry) => entry.payload?.stationDiagnostics?.lastUpdatedAt || entry.payload?.current?.time)
            .filter(Boolean)
            .sort()
            .slice(-1)[0] || null,
          confidence: Math.round(meanOfValues(payloadEntries.map((entry) => entry.payload?.stationDiagnostics?.confidence)) || payloadEntries[0].confidence || 70)
        }
      : null;
    if (!selection?.candidates?.length || selection.method === 'global-model' || !selection?.stations?.length) {
      const error = createError('unsupported', 'METAR/NOAA sem aeroporto observacional funcional dentro de 150 km para este ponto.');
      error.stationSelection = normalizeStationSelection({
        ...baseSelection,
        apiStatus: 'offline-controlled',
        failureReason: getFailureReason(error),
        stations: payloadEntries.map((entry) => entry.station)
      });
      throw error;
    }
    ...
    return attachStationSelection(payloadEntries[0].payload, {
      ...selection,
      method: 'nearest',
      stations: [payloadEntries[0].station],
      primaryDistanceKm: payloadEntries[0].station.distanceKm,
      distanceWeight: getStationDistanceWeight(payloadEntries[0].station.distanceKm),
      apiStatus: payloadEntries[0].payload?.stationDiagnostics?.apiStatus || selection.apiStatus,
      latencyMs: payloadEntries[0].payload?.stationDiagnostics?.latencyMs || selection.latencyMs,
      lastUpdatedAt: payloadEntries[0].payload?.stationDiagnostics?.lastUpdatedAt || selection.lastUpdatedAt,
      confidence: payloadEntries[0].payload?.stationDiagnostics?.confidence || selection.confidence
    });
  }
  ...
  async function findNearestWeatherStation(lat, lon, options = {}) {
    const location = {
      lat: Number(lat),
      lon: Number(lon),
      name: options.name || 'Ponto pesquisado'
    };

    if (!Number.isFinite(location.lat) || !Number.isFinite(location.lon)) {
      throw createError('unsupported', 'Coordenadas invalidas para localizar base meteorologica.');
    }

    const cacheKey = buildCacheKey('nearestWeatherStation', location);
    const cached = readCache(cacheKey);
    const cacheAge = cached ? Date.now() - cached.timestamp : Number.POSITIVE_INFINITY;
    if (!options.force && cached && cacheAge <= CACHE_TTL.stationSelection && cached.value) {
      return cached.value;
    }

    const selectionJobs = [
      isLikelyBrazilCoverage(location)
        ? resolveInmetStationSelection(location)
        : Promise.resolve(createVirtualStationSelection('inmet', 'INMET', 'unavailable')),
      resolveMetarStationSelection(location),
      resolveMeteostatStationSelection(location)
    ];

    const resolved = await Promise.allSettled(selectionJobs);
    const selections = resolved
      .map((result) => result.status === 'fulfilled' ? result.value : null)
      .filter(Boolean);
    const choices = selections
      .flatMap((selection) => buildStationChoices(selection))
      .sort((a, b) => b.score - a.score);
    const best = choices[0] || null;
    const fallback = createVirtualStationSelection('openMeteo', 'Open-Meteo', 'global-model');
    const result = best
      ? {
          sourceKey: best.selection.sourceKey,
          label: best.selection.label,
          priority: best.selection.priority,
          method: best.selection.method,
          station: best.station,
          distanceKm: best.distanceKm,
          distanceWeight: getStationDistanceWeight(best.distanceKm),
          searchRadiusKm: best.selection.searchRadiusKm || STATION_DISCOVERY_RADIUS_KM,
          score: roundTo(best.score, 1),
          selection: normalizeStationSelection({
            ...best.selection,
            stations: best.selection.stations?.length ? best.selection.stations : [best.station].filter(Boolean)
          }),
          message: buildStationUsageText(best.selection, best.station, best.distanceKm)
        }
      : {
          ...
          station: null,
          distanceKm: null,
          distanceWeight: fallback.distanceWeight,
          searchRadiusKm: fallback.searchRadiusKm,
          score: 40,
          selection: fallback,
          message: 'Sem base observacional proxima; usando modelos por coordenadas.'
        };
  ...
  async function findNearestWorkingStation(lat, lon, options = {}) {
    const location = {
      lat: Number(lat),
      lon: Number(lon),
      name: options.name || 'Ponto pesquisado'
    ...
    if (!Number.isFinite(location.lat) || !Number.isFinite(location.lon)) {
      throw createError('unsupported', 'Coordenadas invalidas para localizar base meteorologica funcional.');
    }

    const cacheKey = buildCacheKey('workingStation', location);
    const cached = readCache(cacheKey);
    const cacheAge = cached ? Date.now() - cached.timestamp : Number.POSITIVE_INFINITY;
    if (!options.force && cached && cacheAge <= STATION_WORKING_CACHE_TTL_MS && cached.value) {
      return cached.value;
    }

    const sources = [
      isLikelyBrazilCoverage(location)
        ? resolveInmetStationSelection(location).catch((error) => createFailedStationSelection('inmet', 'INMET', error))
        : Promise.resolve(createVirtualStationSelection('inmet', 'INMET', 'unavailable')),
      resolveMetarStationSelection(location).catch((error) => createFailedStationSelection('metarNoaa', 'METAR/NOAA', error)),
      resolveMeteostatStationSelection(location).catch((error) => createFailedStationSelection('meteostat', 'Meteostat', error))
    ];

    const selections = (await Promise.all(sources)).filter(Boolean);
    const probes = await Promise.allSettled(
      selections.flatMap((selection) => getProbeCandidatesForSelection(selection, location, options))
    );
    ...
    const result = best
      ? {
          ...
          selection: normalizeStationSelection({
            ...
            priority: best.priority,
            method: best.method,
            primaryDistanceKm: best.distanceKm,
            latencyMs: best.latencyMs,
            lastUpdatedAt: best.lastUpdatedAt,
            apiStatus: best.apiStatus,
            confidence: best.confidence,
            stations: [best.station]
          }),
          message: buildStationUsageText({
            label: best.label,
            method: best.method
          }, best.station, best.distanceKm)
        }
      : {
          sourceKey: 'openMeteo',
          label: 'Open-Meteo',
          ...
          latencyMs: null,
          lastUpdatedAt: null,
          apiStatus: 'fallback-model',
          failureReason: 'Sem estacao funcional recente; fallback por modelos meteorologicos.',
          confidence: 52,
          selection: createVirtualStationSelection('openMeteo', 'Open-Meteo', 'global-model'),
          message: 'Sem estacao funcional recente; usando modelos meteorologicos.'
        };
  ...
  function createFailedStationSelection(sourceKey, label, error) {
  ...
  function getProbeCandidatesForSelection(selection, location, options = {}) {
    const normalized = normalizeStationSelection(selection);
    if (!normalized || normalized.method === 'unavailable') return [];
    const sourceKey = normalized.sourceKey;
    const rawCandidates = Array.isArray(selection.candidates) && selection.candidates.length
      ? selection.candidates
      : (normalized.stations || []);
    const candidates = rankStationCandidatesByHealth(rawCandidates, sourceKey)
      .slice(0, options.limit || (sourceKey === 'inmet' ? INMET_STATION_CANDIDATE_LIMIT : METAR_STATION_CANDIDATE_LIMIT));

    return candidates.map((station) => probeStationCandidate(sourceKey, normalized.label, station, location, normalized));
  }

  async function probeStationCandidate(sourceKey, label, station, location, selection) {
    if (sourceKey === 'inmet') {
      return probeTimedStation(sourceKey, label, station, selection, async () => {
        const rows = await fetchInmetStationSeries(station);
        const payload = normalizeInmetStationResponse(rows, station);
        return payload.current;
      });
    }

    if (sourceKey === 'metarNoaa') {
      return probeTimedStation(sourceKey, label, station, selection, async () => {
        const reports = await fetchMetarReports([station.id], { allowCachedFallback: true });
        const report = reports.get(station.id);
        if (!report) {
          throw createStationError('empty_response', sourceKey, station, 'METAR/NOAA sem METAR recente para este ICAO.');
        }
        const payload = normalizeMetarStationResponse(report, station);
        return payload.current;
      });
    ...
    const health = getStationHealth(sourceKey, station.id);
    const confidence = calculateStationConfidence({
      station,
      sourceKey,
      selection,
    ...
    return {
      working: Boolean(health && health.status !== 'offline-controlled'),
      sourceKey,
      label,
  ...
  async function probeTimedStation(sourceKey, label, station, selection, fetcher) {
    const startedAt = performance.now();
    try {
      const current = await fetcher();
      validateStationCurrent(current, {
        sourceKey,
        station,
        requireHumidity: sourceKey === 'inmet'
      });
      const latencyMs = Math.max(1, Math.round(performance.now() - startedAt));
      const lastUpdatedAt = normalizeTimeValue(current?.time) || new Date().toISOString();
      const apiStatus = latencyMs > STATION_HIGH_LATENCY_MS ? 'degraded-latency' : 'online';
      const confidence = calculateStationConfidence({
        station,
        sourceKey,
        selection,
      ...
      markStationHealth(sourceKey, station, {
        status: apiStatus,
        latencyMs,
        lastUpdatedAt,
        confidence,
  ...
  function buildStationChoices(selection) {
    const normalized = normalizeStationSelection(selection);
    if (!normalized || normalized.method === 'unavailable') return [];

    const stations = normalized.stations?.length
      ? normalized.stations
      : (Array.isArray(selection.candidates) ? selection.candidates : [])
        .map((candidate) => normalizeStationCandidate(candidate, null))
        .filter(Boolean)
        .slice(0, STATION_BLEND_LIMIT);

    if (!stations.length) {
      return [{
        selection: normalized,
        station: null,
        distanceKm: normalized.primaryDistanceKm,
        score: scoreStationChoice(normalized, null, normalized.primaryDistanceKm)
      }];
    }
    ...
    return stations.map((station) => {
      const distanceKm = Number.isFinite(station.distanceKm)
        ? station.distanceKm
        : normalized.primaryDistanceKm;
      return {
        selection: normalized,
        station,
        distanceKm,
        score: scoreStationChoice(normalized, station, distanceKm)
      };
    });
  }

  function scoreStationChoice(selection, station, distanceKm) {
    const distance = Number.isFinite(distanceKm) ? distanceKm : STATION_DISCOVERY_RADIUS_KM;
    const priorityBonus = selection.priority === 'official'
      ? 10
      : selection.priority === 'airport'
        ? 6
        : selection.priority === 'meteostat'
          ? 4
          : 0;
    const methodBonus = selection.method === 'nearest'
      ? 10
      : selection.method === 'blend'
        ? 4
        : selection.method === 'fallback-point'
          ? -8
          : -18;
    const distanceScore = getStationDistanceWeight(distance) * 26 - Math.min(38, distance * 0.18);
    const stationBonus = station ? 6 : -16;

    return 60 + priorityBonus + methodBonus + distanceScore + stationBonus;
  }

  function buildStationUsageText(selection, station, distanceKm) {
    if (!selection || !station) {
      return 'Base meteorologica: modelos por coordenadas';
    }

    const distanceText = Number.isFinite(distanceKm) ? `${formatNumber(distanceKm, 1)} km` : 'distancia indefinida';
    const methodText = selection.method === 'blend'
      ? 'media de bases proximas'
      : selection.method === 'nearest'
        ? 'base mais proxima'
        : 'fallback por coordenadas';

    return `Base usada: ${selection.label} ${station.name || station.id} (${distanceText}, ${methodText})`;
  }

  function createStationError(code, sourceKey, station, message) {
    const error = createError(code, message);
    error.sourceKey = sourceKey;
    error.stationId = station?.id || '';
    return error;
  }

  function getFailureReason(error) {
    if (!error) return '';
    if (error.code === 'empty_response') return 'resposta vazia';
    if (error.code === 'invalid_json') return 'JSON invalido';
    if (error.code === 'network') return 'timeout ou falha de rede';
    if (error.code === 'provider_offline') return error.statusCode ? `HTTP ${error.statusCode}` : 'erro HTTP';
    if (error.code === 'stale_station') return 'estacao sem atualizacao recente';
    if (error.code === 'invalid_station_data') return 'dados essenciais ausentes';
    if (error.code === 'high_latency') return 'latencia alta';
    if (error.message) return error.message;
    return 'falha desconhecida';
  }

  function getStationHealthKey(sourceKey, stationId) {
    return `stationHealth:${sourceKey}:${String(stationId || '').trim().toUpperCase()}`;
  }

  function getStationHealth(sourceKey, stationId) {
    const cached = readCache(getStationHealthKey(sourceKey, stationId));
    if (!cached) return null;
    const ageMs = Date.now() - cached.timestamp;
    if (ageMs > STATION_HEALTH_CACHE_TTL_MS) return null;
    return {
      ...(cached.value || {}),
      ageMs
    };
  ...
  function markStationHealth(sourceKey, station, metadata = {}) {
    if (!station?.id) return;
    writeCache(getStationHealthKey(sourceKey, station.id), {
      stationId: station.id,
      sourceKey,
      status: metadata.status || 'unknown',
      latencyMs: Number.isFinite(metadata.latencyMs) ? metadata.latencyMs : null,
      lastUpdatedAt: metadata.lastUpdatedAt || null,
      confidence: Number.isFinite(metadata.confidence) ? metadata.confidence : null,
      failureReason: metadata.failureReason || '',
      updatedAt: new Date().toISOString()
    });
  }

  function isStationRecentlyOffline(sourceKey, station) {
    const health = getStationHealth(sourceKey, station?.id);
    return Boolean(health?.status === 'offline-controlled' && health.ageMs <= STATION_HEALTH_CACHE_TTL_MS);
  }

  function rankStationCandidatesByHealth(candidates, sourceKey) {
    return (Array.isArray(candidates) ? candidates : [])
      .map((candidate) => {
        const station = normalizeStationCandidate(candidate, null);
        if (!station) return null;
        const health = getStationHealth(sourceKey, station.id);
        const offlinePenalty = health?.status === 'offline-controlled' ? 10000 : 0;
        const latencyPenalty = Number.isFinite(health?.latencyMs) ? health.latencyMs / 12 : 80;
        const recencyPenalty = health?.lastUpdatedAt
          ? Math.min(300, Math.max(0, Date.now() - new Date(health.lastUpdatedAt).getTime()) / 60000)
          : 120;
        return {
          ...station,
          apiStatus: health?.status || station.apiStatus || 'unknown',
          latencyMs: health?.latencyMs ?? station.latencyMs ?? null,
          lastUpdatedAt: health?.lastUpdatedAt || station.lastUpdatedAt || null,
          failureReason: health?.failureReason || station.failureReason || '',
          sortScore: station.distanceKm + latencyPenalty + recencyPenalty + offlinePenalty
        };
      })
      .filter(Boolean)
      .filter((station) => !isStationRecentlyOffline(sourceKey, station))
      .sort((a, b) => a.sortScore - b.sortScore);
  }

  function validateStationCurrent(current, options = {}) {
    const sourceKey = options.sourceKey || 'station';
    const station = options.station || {};
    const missing = [];

    if (!Number.isFinite(current?.temperature)) missing.push('temperatura');
    if (!Number.isFinite(current?.windSpeed)) missing.push('vento');
    if (options.requireHumidity && !Number.isFinite(current?.humidity)) missing.push('umidade');
    ...
    if (missing.length) {
      throw createStationError(
        ...
        sourceKey,
        station,
        `${options.label || sourceKey}: ${station.id || 'estacao'} sem ${missing.join(', ')}.`
      );
    }

    const updatedAt = normalizeTimeValue(current?.time);
    const ageMs = updatedAt ? Math.max(0, Date.now() - new Date(updatedAt).getTime()) : Number.POSITIVE_INFINITY;
    if (!Number.isFinite(ageMs) || ageMs > STATION_OBSERVATION_MAX_AGE_MS) {
      throw createStationError(
        'stale_station',
        sourceKey,
        station,
        `${options.label || sourceKey}: ${station.id || 'estacao'} sem atualizacao recente.`
      );
    }
  }

  function calculateStationConfidence(input = {}) {
    const station = input.station || {};
    const distanceKm = Number.isFinite(station.distanceKm) ? station.distanceKm : input.distanceKm;
    const latencyMs = Number(input.latencyMs);
    const updatedAt = normalizeTimeValue(input.lastUpdatedAt || input.current?.time);
    const ageMs = updatedAt ? Math.max(0, Date.now() - new Date(updatedAt).getTime()) : STATION_OBSERVATION_MAX_AGE_MS;
    const distancePenalty = Number.isFinite(distanceKm) ? Math.min(28, distanceKm * 0.16) : 16;
    const latencyPenalty = Number.isFinite(latencyMs)
      ? latencyMs <= 700
        ? 0
        : latencyMs <= STATION_HIGH_LATENCY_MS
          ? (latencyMs - 700) / 120
          : 12
      : 8;
    const agePenalty = ageMs <= 30 * 60 * 1000
      ? 0
      : ageMs <= 2 * 60 * 60 * 1000
        ? 4
        : ageMs <= STATION_OBSERVATION_MAX_AGE_MS
          ? 11
          : 25;
    const statusPenalty = input.apiStatus === 'online'
      ? 0
      : input.apiStatus === 'degraded-latency'
        ? 6
        : input.apiStatus === 'fallback-model'
          ? 14
          : 10;
    const priorityBonus = input.selection?.priority === 'official'
      ? 5
      : input.selection?.priority === 'airport'
        ? 3
        : 0;

    return clamp(Math.round(96 + priorityBonus - distancePenalty - latencyPenalty - agePenalty - statusPenalty), 35, 98);
  }
  ...
  async function fetchMetarReports(ids, options = {}) {
    ...
    const cacheKey = `metarReports:${stationIds.join(',')}`;
    const cached = readCache(cacheKey);
    const cacheAge = cached ? Date.now() - cached.timestamp : Number.POSITIVE_INFINITY;
    if (cached && cacheAge <= METAR_REPORT_CACHE_TTL_MS && Array.isArray(cached.value?.rows)) {
      const map = buildMetarReportMap(cached.value.rows);
      map.latencyMs = cached.value.latencyMs ?? null;
      map.cacheStatus = 'cache';
      return map;
    }
    ...
    const startedAt = performance.now();
    try {
      data = await fetchJson(`${METAR_DATA_URL}?${params.toString()}`, {
        timeoutMs: CORE_PROVIDER_TIMEOUT_MS,
        retries: 1
      });
    } catch (error) {
      if (options.allowCachedFallback && cached && Array.isArray(cached.value?.rows)) {
        const map = buildMetarReportMap(cached.value.rows);
        map.latencyMs = cached.value.latencyMs ?? null;
        map.cacheStatus = 'cache';
        map.failureReason = getFailureReason(error);
        return map;
      }
    ...
    const map = buildMetarReportMap(rows);
    map.latencyMs = latencyMs;
    map.cacheStatus = latencyMs > STATION_HIGH_LATENCY_MS ? 'degraded-latency' : 'live';
    return map;
  }

  function buildMetarReportMap(rows) {
  ...
  async function fetchObservationPayloadFromSelection(selection, payloadFetcher, options = {}) {
    ...
    const sourceKey = selection.sourceKey || options.sourceKey || '';
    const rankedCandidates = rankStationCandidatesByHealth(selection.candidates, sourceKey);
    const candidatePool = rankedCandidates;
    const probeLimit = options.probeLimit || (selection.method === 'blend' ? STATION_BLEND_LIMIT : Math.max(STATION_BLEND_LIMIT, 3));
    const stationsToTry = candidatePool.slice(0, Math.max(1, probeLimit));

    if (!stationsToTry.length) {
      throw createError('unsupported', options.nearestErrorMessage || 'Sem estacoes funcionais para o ponto selecionado.');
    }

    const settled = await Promise.allSettled(
      stationsToTry.map(async (station) => {
        const startedAt = performance.now();
        try {
          const payload = await payloadFetcher(station);
          const latencyMs = Number.isFinite(payload?.stationDiagnostics?.latencyMs)
            ? payload.stationDiagnostics.latencyMs
          ...
          markStationHealth(sourceKey, enrichedStation, {
            status: apiStatus,
            latencyMs,
            lastUpdatedAt,
            confidence,
    ...
    if (!validPayloads.length) {
      const error = failed[0] || createError('unsupported', options.nearestErrorMessage || 'Sem dados validos nas estacoes selecionadas.');
      error.stationSelection = normalizeStationSelection({
        ...selection,
        apiStatus: 'offline-controlled',
        failureReason: getFailureReason(error),
        stations: stationsToTry.slice(0, 3)
      });
      throw error;
    }

    const shouldBlend = selection.method === 'blend' && validPayloads.length > 1;
    if (shouldBlend) {
      const selected = validPayloads.slice(0, STATION_BLEND_LIMIT);
      return blendStationPayloadEntries(selected, {
        ...selection,
        method: 'blend',
        apiStatus: selected.some((entry) => entry.apiStatus === 'degraded-latency') ? 'degraded-latency' : 'online',
        latencyMs: Math.round(meanOfValues(selected.map((entry) => entry.latencyMs)) || selected[0].latencyMs),
        lastUpdatedAt: selected.map((entry) => entry.lastUpdatedAt).filter(Boolean).sort().slice(-1)[0] || null,
        confidence: Math.round(meanOfValues(selected.map((entry) => entry.confidence)) || selected[0].confidence),
        stations: selected.map((entry) => entry.station),
        primaryDistanceKm: selected[0].station.distanceKm
      }, {
    ...
    const best = validPayloads[0];
    return attachStationSelection(best.payload, {
      ...selection,
      method: 'nearest',
      stations: [best.station],
      primaryDistanceKm: best.station.distanceKm,
      distanceWeight: getStationDistanceWeight(best.station.distanceKm),
      apiStatus: best.apiStatus,
      latencyMs: best.latencyMs,
      lastUpdatedAt: best.lastUpdatedAt,
      confidence: best.confidence,
  ...
  function normalizeStationSelection(selection) {
    ...
    return {
      ...
      apiStatus: String(selection.apiStatus || selection.status || ''),
      failureReason: String(selection.failureReason || ''),
      latencyMs: toFiniteNumber(selection.latencyMs),
      lastUpdatedAt: normalizeTimeValue(selection.lastUpdatedAt) || null,
      confidence: toFiniteNumber(selection.confidence),
      ...
      stations: (Array.isArray(selection.stations) ? selection.stations : [])
        .map((station) => ({
          ...
          latencyMs: toFiniteNumber(station.latencyMs),
          lastUpdatedAt: normalizeTimeValue(station.lastUpdatedAt) || null,
          apiStatus: String(station.apiStatus || ''),
          failureReason: String(station.failureReason || ''),
          confidence: toFiniteNumber(station.confidence)
        }))
  ...
  function normalizeMetarStationResponse(entry, station) {
    ...
    const windSpeed = knotsToKmh(toFiniteNumber(entry?.wspd));
    const current = {
      time,
  ...
  function buildInmetObservedSnapshot(entry) {
    ...
    const windSpeed = valueOr(toFiniteNumber(entry?.VEN_VEL), toFiniteNumber(entry?.VEL_VENTO_MED), null);
  ...
  function normalizeOpenMeteoResponse(data, label) {
    ...
    const current = {
      ...
      visibilityKm: valueOr(currentFromHourly.visibilityKm, estimateVisibilityKm(data?.current?.cloud_cover, data?.current?.precipitation), 10),
      uvIndex: valueOr(currentFromHourly.uvIndex, data?.daily?.uv_index_max?.[0], 0),
  ...
  function buildWeatherBundle(successfulRuns, allRuns, location) {
    ...
    const realRunCount = fusionRuns.filter((run) => run.type !== 'demo' && run.type !== 'cache' && run.providerKey !== 'demoWeather' && run.providerKey !== 'savedCache').length;
    const defaultConfidence = realRunCount >= 3
      ? 94
      : realRunCount === 2
        ? 88
        : realRunCount === 1
    ...
    const stationUsage = buildUsedWeatherStationSummary(fusionRuns);
    ...
    const visibleProviderTotal = getEnabledProviders(location).filter((provider) => provider.visible !== false).length;
    ...
    const bundle = {
      ...
      daily,
      analytics,
      stationSelection: stationUsage?.selection || null,
  ...
  function buildUsedWeatherStationSummary(runs) {
    const stationRuns = dedupeFallbackRuns(runs)
      .filter((run) => isValidProviderRun(run) && run.stationSelection)
      .map((run) => {
        const selection = normalizeStationSelection(run.stationSelection);
        const station = selection?.stations?.[0] || null;
        const distanceKm = Number.isFinite(station?.distanceKm)
          ? station.distanceKm
          : selection?.primaryDistanceKm;
        const latencyMs = valueOr(station?.latencyMs, selection?.latencyMs, run.latencyMs);
        const dataAgeMs = Math.max(0, Date.now() - new Date(run.current?.time || run.fetchedAt || Date.now()).getTime());
        const recencyScore = dataAgeMs <= 30 * 60 * 1000
          ? 18
          : dataAgeMs <= 2 * 60 * 60 * 1000
            ? 10
            : dataAgeMs <= 6 * 60 * 60 * 1000
              ? 2
              : -10;
        const distanceScore = getStationDistanceWeight(distanceKm) * 24;
        const qualityScore = getProviderDataQualityScore(run) * 18;
        const priorityScore = selection?.priority === 'official'
          ? 10
          : selection?.priority === 'airport'
            ? 6
            : 3;
        const methodScore = selection?.method === 'nearest'
          ? 8
          : selection?.method === 'blend'
            ? 3
            : selection?.method === 'fallback-point'
        ...
        return {
          ...
          latencyMs,
          dataAgeMs,
          score: 40 + recencyScore + distanceScore + qualityScore + priorityScore + methodScore
        };
      })
      .filter((entry) => entry.selection);

    if (!stationRuns.length) return null;

    stationRuns.sort((a, b) => b.score - a.score);
    const best = stationRuns[0];
    const stationLabel = best.station
      ? `${best.selection.label} ${best.station.name || best.station.id}`
      : best.run.label;

    return {
      sourceKey: best.selection.sourceKey,
      label: best.selection.label,
      providerLabel: best.run.label,
      method: best.selection.method,
      station: best.station,
      distanceKm: Number.isFinite(best.distanceKm) ? best.distanceKm : null,
      latencyMs: Number.isFinite(best.latencyMs) ? best.latencyMs : null,
      lastUpdatedAt: best.station?.lastUpdatedAt || best.selection.lastUpdatedAt || best.run.current?.time || null,
      apiStatus: best.station?.apiStatus || best.selection.apiStatus || best.run.apiStatus || '',
      failureReason: best.station?.failureReason || best.selection.failureReason || best.run.failureReason || '',
      dataAgeMs: best.dataAgeMs,
      score: roundTo(best.score, 1),
      selection: best.selection,
      message: buildStationUsageText(best.selection, best.station, best.distanceKm),
      shortLabel: Number.isFinite(best.distanceKm)
        ? `${stationLabel} (${formatNumber(best.distanceKm, 1)} km)`
        : stationLabel
    };
  }
  ...
  function getProviderDataQualityScore(run) {
    ...
    const currentCoverage = fields.filter((field) => Number.isFinite(run.current?.[field])).length / fields.length;
  ...
  function finalizeProviderRun(provider, data, startedAt) {
    ...
    const run = {
      ...
      apiStatus: sanitized.stationSelection?.apiStatus || (sanitized.partial ? 'partial' : 'online'),
      failureReason: sanitized.stationSelection?.failureReason || '',
      current: sanitized.current,
      hourly: sanitized.hourly,
      daily: sanitized.daily
    };
  ...
  function createProviderFailureRun(provider, error, startedAt) {
    return {
      ...
      latencyMs: Math.max(1, Math.round(performance.now() - startedAt)),
      weight: 0,
      stationSelection: normalizeStationSelection(error?.stationSelection),
      apiStatus: 'offline-controlled',
      failureReason: getFailureReason(error),
      current: null,
      hourly: [],
  ...
  function stripTransientProviderFields(run) {
    return {
      ...
      apiStatus: run.apiStatus || run.stationSelection?.apiStatus || '',
      failureReason: run.failureReason || run.stationSelection?.failureReason || '',
      current: run.current,
      hourly: run.hourly,
      daily: run.daily
    };
  ...
  function computeProviderWeight(run) {
    ...
    const latencyFactor = Number.isFinite(run.latencyMs)
      ...
      : 0.9;
    const stationLatency = valueOr(run.stationSelection?.latencyMs, run.latencyMs);
    const stationLatencyFactor = Number.isFinite(stationLatency)
      ? stationLatency > STATION_HIGH_LATENCY_MS
        ? 0.68
        : clamp(1 - (stationLatency / STATION_HIGH_LATENCY_MS) * 0.08, 0.86, 1)
      : 0.92;
    ...
    const stationAvailabilityFactor = run.stationSelection?.apiStatus === 'offline-controlled'
      ? 0.1
      : run.stationSelection?.apiStatus === 'degraded-latency'
        ? 0.72
        : run.stationSelection?.apiStatus === 'cache-fallback'
          ? 0.62
          : 1;
    ...
    const resolvedWeight = baseWeight
      ...
      * stationLatencyFactor
      * stationAvailabilityFactor
      * stationModeFactor
  ...
  function buildModelAgreement(successfulRuns, defaultConfidence) {
    ...
    const ignoredModelLabels = [...new Set(aggregateSamples.flatMap((entry) => entry.ignoredModelLabels || []))];
    ...
    return {
      ...
      activeModelCount: comparisonRuns.length,
      comparedModelKeys: comparisonRuns.map((run) => run.providerKey),
      ignoredModelCount: ignoredModelLabels.length,
      ignoredModelLabels,
      byHour,
      byDay
    };
  ...
  function summarizeModelSpread(samples, defaultConfidence) {
    ...
    const filtered = filterModelOutliers(comparable);
    const stableSamples = filtered.samples.length >= 2 ? filtered.samples : comparable;
    const tempSpread = calculateSpread(stableSamples.map((sample) => sample.temperature));
    const windSpread = calculateSpread(stableSamples.map((sample) => sample.windSpeed));
    const rainSpread = calculateSpread(stableSamples.map((sample) => sample.rainProbability));
    ...
    const tempExcess = Math.max(0, valueOr(tempSpread, 0) - MODEL_DIVERGENCE_THRESHOLDS.temperature.medium);
    const windExcess = Math.max(0, valueOr(windSpread, 0) - MODEL_DIVERGENCE_THRESHOLDS.windSpeed.medium);
    const rainExcess = Math.max(0, valueOr(rainSpread, 0) - MODEL_DIVERGENCE_THRESHOLDS.rainProbability.medium);
    const lowDivergenceBonus = divergenceLevel === 'baixo' && stableSamples.length >= 3 ? 3 : 0;
    const outlierPenalty = filtered.ignored.length ? 1.5 : 0;
    const confidence = clamp(Math.round(
      (stableSamples.length >= 3 ? 95 : 90)
      + lowDivergenceBonus
      - tempExcess * 1.6
      - windExcess * 0.55
      - rainExcess * 0.08
      - Math.max(0, 3 - stableSamples.length) * 3
      - outlierPenalty
    ), 58, 98);
    ...
    return {
      ...
      rawSampleCount: comparable.length,
      ignoredModelCount: filtered.ignored.length,
      ignoredModelLabels: filtered.ignored.map((sample) => sample.label).filter(Boolean)
    };
  }

  function filterModelOutliers(samples) {
    const list = (Array.isArray(samples) ? samples : []).filter((sample) => sample && sample.weight > 0);
    if (list.length < 3) {
    ...
    [
      { key: 'temperature', threshold: MODEL_DIVERGENCE_THRESHOLDS.temperature.high },
      { key: 'windSpeed', threshold: MODEL_DIVERGENCE_THRESHOLDS.windSpeed.high },
      { key: 'rainProbability', threshold: MODEL_DIVERGENCE_THRESHOLDS.rainProbability.high }
      ...
      list.forEach((sample) => {
        if (Number.isFinite(sample[key]) && Math.abs(sample[key] - median) > threshold) {
          ignored.add(sample);
        }
      });
    });

    const samplesKept = list.filter((sample) => !ignored.has(sample));
    if (samplesKept.length < 2) {
      return { samples: list, ignored: [] };
    }
  ...
  function normalizeProviderAliasToKey(value) {
    ...
    if (normalized.includes('demo')) return 'demoWeather';
    ...
    if (normalized.includes('metar')) return 'metarNoaa';
    if (normalized.includes('noaa/nws') || normalized.includes('nws') || normalized.includes('noaa')) return 'noaa';
  ...
  function buildSourceConsensus(successfulRuns) {
    const consensusRuns = dedupeFallbackRuns(successfulRuns).filter((run) => (
      isDisplayableLiveProvider(run)
      && run.type !== 'demo'
      && run.type !== 'cache'
    ...
    const comparableMetrics = {
      ...
      windGusts: {
        ...
        warningSpread: 24,
        scoreFactor: 0.35,
        resolver: (run) => valueOr(run.current?.windGusts, run.daily?.[0]?.windGusts)
      },
      rainProbability: {
        label: 'chuva',
        unit: '%',
      ...
      precipitation: {
        ...
        warningSpread: 8,
        scoreFactor: 0.7,
        resolver: (run) => valueOr(run.current?.precipitation, run.daily?.[0]?.precipitation)
      },
      humidity: {
        label: 'umidade',
        unit: '%',
      ...
      visibilityKm: {
        ...
        warningSpread: 12,
        scoreFactor: 0.9,
        resolver: (run) => valueOr(run.current?.visibilityKm, run.daily?.[0]?.visibilityKm)
      }
    };
    ...
    Object.entries(comparableMetrics).forEach(([key, definition]) => {
      const rawSamples = consensusRuns
        .map((run) => ({
          label: run.label,
          weight: run.weight,
      ...
      const filtered = filterConsensusOutliers(rawSamples, definition);
      ...
      const mean = weightedAverage(samples.map((sample) => ({
        value: sample.value,
        weight: Number.isFinite(sample.weight) && sample.weight > 0 ? sample.weight : 1
      }))) || meanOfValues(values);
      const lightRain = (key === 'rainProbability' && mean <= 38) || (key === 'precipitation' && mean <= 2.5);
      const safeSpread = lightRain ? definition.safeSpread * 1.5 : definition.safeSpread;
      const warningSpread = lightRain ? definition.warningSpread * 1.45 : definition.warningSpread;
      const status = spread > warningSpread ? 'warning' : spread > safeSpread ? 'watch' : 'safe';
      ...
      score -= Math.min(status === 'warning' ? 18 : 5, spread * definition.scoreFactor);
    });

    score -= Math.max(0, 3 - consensusRuns.length) * 6;
    if (ignoredSourceLabels.size) {
      score -= 1.5;
    }
    ...
    if (!Object.keys(metrics).length) {
      return {
        ...
        leadMessage: 'Ainda ha poucas fontes comparaveis para medir concordancia com seguranca.',
        windConflict: false,
        rainConflict: false,
        thermalConflict: false,
        humidityConflict: false,
        ignoredSourceCount: ignoredSourceLabels.size,
        ignoredSourceLabels: [...ignoredSourceLabels],
        metrics
      };
    }

    const windConflict = valueOr(metrics.windSpeed?.spread, 0) > 18;
    const gustConflict = valueOr(metrics.windGusts?.spread, 0) > 24;
    const rainConflict = valueOr(metrics.rainProbability?.spread, 0) > 58 || valueOr(metrics.precipitation?.spread, 0) > 8;
    const thermalConflict = valueOr(metrics.temperature?.spread, 0) > 7;
    const humidityConflict = valueOr(metrics.humidity?.spread, 0) > 30;
    const pressureConflict = valueOr(metrics.pressure?.spread, 0) > 11;
    const visibilityConflict = valueOr(metrics.visibilityKm?.spread, 0) > 12;

    let overallStatus = 'safe';
    if (score < 66 || windConflict || gustConflict || rainConflict || visibilityConflict) {
      overallStatus = 'warning';
    }
    ...
    } else if (gustConflict) {
      leadMessage = 'Rajadas variam bastante entre fontes. Mantenha margem extra para pouso e retorno.';
    } else if (rainConflict) {
      leadMessage = 'Probabilidade de chuva varia bastante entre fontes. Reforce contingência e plano de pouso.';
    } else if (thermalConflict) {
      leadMessage = 'Temperatura varia entre fontes. Considere variação térmica maior no planejamento.';
    } else if (humidityConflict) {
      leadMessage = 'Umidade varia bastante entre fontes. Monitore condensação e eletrônica sensível.';
    }

    if (!windConflict && !gustConflict && !rainConflict && !thermalConflict && !humidityConflict && pressureConflict) {
      leadMessage = 'Pressao varia entre fontes. Monitore mudancas bruscas no curto prazo.';
    } else if (!windConflict && !gustConflict && !rainConflict && !thermalConflict && !humidityConflict && visibilityConflict) {
      leadMessage = 'Visibilidade varia entre fontes. Confirme condicao visual antes de voar.';
    }
    ...
    return {
      ...
      shortLabel: overallStatus === 'safe' ? 'Alta concordancia' : 'Variacao moderada',
      leadMessage,
      windConflict,
      gustConflict,
      rainConflict,
  ...
  function filterConsensusOutliers(samples, definition) {
    ...
    const threshold = definition.warningSpread;
    const ignored = list.filter((sample) => Math.abs(sample.value - median) > threshold);
    const samplesKept = list.filter((sample) => !ignored.includes(sample));
  ...
  function computeConfidence(successfulRuns, modelAgreement, sourceConsensus, defaultConfidence) {
    ...
    const realRuns = activeRuns.filter((run) => run.type !== 'demo' && run.type !== 'cache' && run.providerKey !== 'demoWeather' && run.providerKey !== 'savedCache');
    const activeCount = activeRuns.length;
    const realCount = realRuns.length;
    const meanLatency = meanOfValues(activeRuns.map((run) => run.latencyMs));
    const stationAdjustments = realRuns
      .map(getStationConfidenceAdjustment)
    ...
    if (stationAdjustments.length) {
      confidence += clamp(meanOfValues(stationAdjustments), -10, 6);
    }

    if (!realCount) {
      return clamp(defaultConfidence, 0, 45);
    }

    const cap = realCount >= 3 ? 98 : realCount === 2 ? 94 : 85;
    return clamp(Math.round(confidence), 45, cap);
  }

  function getStationConfidenceAdjustment(run) {
    const selection = normalizeStationSelection(run?.stationSelection);
    if (!selection) return null;

    const distance = Number.isFinite(selection.primaryDistanceKm)
      ? selection.primaryDistanceKm
      : selection.stations?.[0]?.distanceKm;
    const distanceAdjustment = !Number.isFinite(distance)
      ? -2
      : distance <= 20
        ? 4
        : distance <= 50
          ...
          : distance <= 100
            ...
            : distance <= 150
              ? -4
              : -7;
    const ageMs = Math.max(0, Date.now() - new Date(run.current?.time || run.fetchedAt || Date.now()).getTime());
    const recencyAdjustment = ageMs <= 30 * 60 * 1000
      ? 2
      : ageMs <= 2 * 60 * 60 * 1000
        ? 0
        : ageMs <= 6 * 60 * 60 * 1000
          ? -4
          : -8;
    const qualityAdjustment = (getProviderDataQualityScore(run) - 0.72) * 6;
    const methodAdjustment = selection.method === 'global-model'
      ? -4
      : selection.method === 'fallback-point'
        ? -3
        : selection.method === 'blend'
    ...
    return distanceAdjustment + recencyAdjustment + qualityAdjustment + methodAdjustment;
  }

  function buildConfidenceNote(confidence, modelAgreement, sourceConsensus, providerCount) {
    if (!modelAgreement.available) {
      return `${providerCount} fontes ativas. O consenso entre fontes validas ganhou mais peso nesta atualizacao.`;
    }
    if (sourceConsensus?.available && sourceConsensus.overallStatus === 'warning') {
      return `Confiabilidade: ${confidence}% com variação perceptível entre fontes, apesar da fusão entre modelos e oficiais.`;
    }
    if (confidence >= 84) {
      return `Confiabilidade: ${confidence}% com alta concordância entre modelos e fontes oficiais brasileiras.`;
    }
    if (confidence >= 68) {
      return `Confiabilidade: ${confidence}% com variação moderada entre modelos e fontes auxiliares.`;
    }
    return `Confiabilidade: ${confidence}% com variação relevante no curto prazo.`;
  }

  function buildSpreadSummary(modelAgreement) {
    if (!modelAgreement.available) {
      return 'Comparacao entre fontes indisponivel';
    }
  ...
  function buildConfidenceNote(confidence, modelAgreement, sourceConsensus, providerCount) {
    if (!modelAgreement.available) {
      return `${providerCount} fontes ativas. O consenso do modo demo e das fontes sem chave ganhou mais peso nesta atualizacao.`;
    }
    if (sourceConsensus?.available && sourceConsensus.overallStatus === 'warning') {
      return `Confiabilidade: ${confidence}% com variacao perceptivel entre as fontes ativas, apesar da fusao ponderada.`;
    }
    if (confidence > 85) {
      return `Confiabilidade: ${confidence}% com alta concordancia entre modelos globais, observacao historica e reforco regional.`;
    }
    if (confidence >= 60) {
      return `Confiabilidade: ${confidence}% com variacao moderada entre modelos e fontes auxiliares.`;
    }
    return `Confiabilidade: ${confidence}% com variacao relevante no curto prazo.`;
  }
  ...
  function buildConfidenceNote(confidence, modelAgreement, sourceConsensus, providerCount) {
    ...
    if (sourceConsensus?.available && sourceConsensus.overallStatus === 'warning') {
      return `Confiabilidade: ${confidence}% com variacao perceptivel entre as fontes ativas, com filtro robusto de outliers e fallback regional em operacao.`;
    }
    if (confidence >= 85) {
    ...
    if (confidence >= 68) {
      return `Confiabilidade: ${confidence}% com variacao moderada entre modelos e fontes auxiliares, mantendo prioridade regional brasileira.`;
    }
    return `Confiabilidade: ${confidence}% com variacao relevante no curto prazo.`;
  }

  function formatIgnoredModelNotice(modelAgreement, sourceConsensus) {
    const labels = [
      ...(Array.isArray(modelAgreement?.ignoredModelLabels) ? modelAgreement.ignoredModelLabels : []),
      ...(Array.isArray(sourceConsensus?.ignoredSourceLabels) ? sourceConsensus.ignoredSourceLabels : [])
    ].filter(Boolean);
    const explicitCount = valueOr(modelAgreement?.ignoredModelCount, 0) + valueOr(sourceConsensus?.ignoredSourceCount, 0);
    const ignoredCount = labels.length ? new Set(labels).size : explicitCount;
    if (!ignoredCount) return '';
    return `${ignoredCount} ${ignoredCount === 1 ? 'modelo ignorado' : 'modelos ignorados'} por inconsistência`;
  }

  function buildConfidenceNote(confidence, modelAgreement, sourceConsensus, providerCount) {
    const ignoredNotice = formatIgnoredModelNotice(modelAgreement, sourceConsensus);
    const ignoredSuffix = ignoredNotice ? ` ${ignoredNotice}.` : '';
    const variationLevel = modelAgreement?.available ? modelAgreement.divergenceLevel : 'baixo';

    if (sourceConsensus?.available && sourceConsensus.overallStatus === 'warning') {
      if (ignoredNotice && variationLevel === 'baixo') {
        return `Confiabilidade: ${confidence}% com consenso entre modelos validos. ${ignoredNotice}.`;
      }
      return `Confiabilidade: ${confidence}% com variacao relevante entre fontes ativas. A fusao ponderada reduziu peso de leituras inconsistentes.${ignoredSuffix}`;
    }
    if (providerCount >= 3 && modelAgreement.available) {
      return variationLevel === 'baixo'
        ? `Confiabilidade: ${confidence}% com alta concordancia entre modelos validos.${ignoredSuffix}`
        : `Confiabilidade: ${confidence}% com variacao controlada entre modelos validos.${ignoredSuffix}`;
    }
    if (providerCount === 2 && modelAgreement.available) {
      return `Confiabilidade: ${confidence}% com duas fontes validas comparadas.${ignoredSuffix}`;
    }
    if (!modelAgreement.available) {
      return `Confiabilidade: ${confidence}% com uma fonte real validada e cache pronto para fallback.${ignoredSuffix}`;
    }
    return `Confiabilidade: ${confidence}% com fusao ponderada entre modelos validos.${ignoredSuffix}`;
  }
  ...
  function buildSpreadSummary(modelAgreement) {
    ...
    const ignoredNotice = formatIgnoredModelNotice(modelAgreement, null);
    const variationText = modelAgreement.divergenceLevel === 'alto' ? 'Variacao alta' : 'Variacao moderada';
    const statusText = ignoredNotice || (modelAgreement.divergenceLevel === 'baixo'
      ? 'Consenso OK'
      : variationText);
    return `ΔT ${formatNumber(modelAgreement.averageTempSpread, 1)}°C • ΔV ${formatNumber(modelAgreement.averageWindSpread, 1)} km/h • ΔC ${formatNumber(modelAgreement.averageRainSpread, 0)}% • ${statusText}`;
  }

  function buildModelSpreadExplanation(modelAgreement, sourceConsensus) {
    if (!modelAgreement?.available) {
      return 'Comparacao entre fontes indisponivel para este ponto.';
    }
    const ignoredNotice = formatIgnoredModelNotice(modelAgreement, sourceConsensus);
    if (ignoredNotice) {
      return `${ignoredNotice}. Calculo principal usa fontes coerentes.`;
    }
    if (modelAgreement.divergenceLevel === 'baixo') {
      return sourceConsensus?.available
        ? `Variacao normal entre modelos nas proximas 24 horas. ${sourceConsensus.summary}.`
        : 'Variacao normal entre modelos nas proximas 24 horas.';
    }
    const level = modelAgreement.divergenceLevel === 'alto' ? 'alta' : 'moderada';
    return sourceConsensus?.available
      ? `Variacao ${level} entre fontes ativas nas proximas 24 horas. ${sourceConsensus.summary}.`
      : `Variacao ${level} entre fontes ativas nas proximas 24 horas.`;
  }
  ...
  function buildAlertStream(bundle) {
    ...
    if (bundle.analytics.confidence < 65) {
      alerts.push({
        tone: 'warning',
        title: 'Modelos com baixa concordancia',
    ...
    if (sourceConsensus?.available && sourceConsensus.overallStatus === 'warning') {
      alerts.push({
        tone: 'warning',
        title: 'Fontes com variacao relevante',
  ...
  function buildClimateInsights(bundle) {
    const insights = [];
    const sourceConsensus = bundle.analytics.sourceConsensus;
    const stationUsage = bundle.analytics.stationUsage;

    if (stationUsage?.shortLabel) {
      insights.push({
        tone: Number.isFinite(stationUsage.distanceKm) && stationUsage.distanceKm > STATION_BLEND_RADIUS_KM ? 'warning' : 'safe',
        text: `Base usada: ${stationUsage.shortLabel}`
      });
    }
    ...
    if (bundle.analytics.confidence >= 84) {
      ...
      insights.push({
        tone: 'warning',
        text: `Confiabilidade de ${bundle.analytics.confidence}% com variacao relevante entre modelos.`
      });
    }
  ...
  function renderClimateTab() {
    ...
    const modelsSummary = getModelsSummary(state.providerRuns);
    const stationPanelText = buildStationPanelText(bundle.analytics.stationUsage || bundle.stationSelection);
    const modelsAndStationSummary = stationPanelText ? `${modelsSummary} - ${stationPanelText}` : modelsSummary;
    const updateAgeText = getUpdateAgeText(state.lastUpdate);
    ...
    dom.currentSummary.textContent = current.description || summaryDetail || 'Clima sincronizado';
    ...
    if (dom.mobileConfidenceFill) {
      dom.mobileConfidenceFill.style.width = `${clamp(bundle.analytics.confidence, 0, 100)}%`;
    }
    dom.confidenceNote.textContent = sourceConsensus?.available ? sourceConsensus.shortLabel : `${getVisibleProviderCount(state.providerRuns)} fontes`;
    ...
    dom.divergenceNote.textContent = sourceConsensus?.available
      ...
      : 'Comparacao entre fontes indisponivel no momento.';
    dom.trendValue.textContent = bundle.analytics.trend.short;
    dom.trendNote.textContent = bundle.analytics.trend.label || bundle.analytics.trend.note;

    dom.providerSummary.textContent = modelsAndStationSummary;
    if (dom.climateHeaderLocation) dom.climateHeaderLocation.textContent = state.location?.name || '--';
    if (dom.climateMobileLocation) dom.climateMobileLocation.textContent = state.location?.name || '--';
    if (dom.climateSidebarRefresh) dom.climateSidebarRefresh.textContent = updateAgeText.replace(/^Atualizado\s*/i, '') || '--';
    if (dom.droneSidebarRefresh) dom.droneSidebarRefresh.textContent = updateAgeText.replace(/^Atualizado\s*/i, '') || '--';
    dom.cacheStatus.textContent = state.cacheInfo.used
      ? state.cacheInfo.stale
        ? `Usando ultimo dado valido salvo (${formatDurationShort(state.cacheInfo.ageMs)})`
        : `Ultimo dado valido salvo (${formatDurationShort(state.cacheInfo.ageMs)})`
      : 'Ao vivo';
    dom.refreshSummary.textContent = `a cada ${state.refreshSeconds}s`;
    dom.refreshExplanation.textContent = state.cacheInfo.used
      ? `Usando \u00faltimo dado valido salvo com ${formatDurationShort(state.cacheInfo.ageMs)} de idade.`
      : 'Atualização ao vivo com cache inteligente de segurança.';

    dom.refreshExplanation.textContent = state.cacheInfo.used
      ? `Usando \u00faltimo dado valido salvo com ${formatDurationShort(state.cacheInfo.ageMs)} de idade.`
      : `${updateAgeText}. ${modelsAndStationSummary}.`;
    ...
    dom.currentConfidenceDetail.textContent = sourceConsensus?.available ? sourceConsensus.shortLabel : 'Fontes ativas';
    ...
    dom.reliabilityExplanation.textContent = `${bundle.analytics.confidenceNote} ${modelsAndStationSummary}. ${updateAgeText}.`;
    dom.modelSpreadScore.textContent = buildSpreadSummary(bundle.modelAgreement);
    dom.modelSpreadExplanation.textContent = buildModelSpreadExplanation(bundle.modelAgreement, sourceConsensus);
    ...
    renderCharts();
    renderMapOverlays();
    window.VentoUXEnhancements?.updateWeatherDashboard?.({
      bundle,
      providerRuns: state.providerRuns,
      cacheInfo: state.cacheInfo,
      location: state.location,
      analysis: buildFlightAnalysis(bundle),
      modelsSummary: modelsAndStationSummary,
      updateAgeText
    });
  }

  function buildStationPanelText(stationUsage) {
    if (!stationUsage) return '';

    const selection = normalizeStationSelection(stationUsage.selection || stationUsage);
    const station = selection?.stations?.[0] || stationUsage.station || null;
    const distanceKm = Number.isFinite(stationUsage.distanceKm)
      ? stationUsage.distanceKm
      : Number.isFinite(station?.distanceKm)
        ? station.distanceKm
        : selection?.primaryDistanceKm;

    if (!selection || !station) {
      return stationUsage.shortLabel ? `Base: ${stationUsage.shortLabel}` : '';
    }

    const stationLabel = [selection.label, station.name || station.id].filter(Boolean).join(' ');
    const details = [];
    if (Number.isFinite(distanceKm)) details.push(`${formatNumber(distanceKm, 1)} km`);
    const latencyMs = valueOr(station.latencyMs, selection.latencyMs, stationUsage.latencyMs);
    if (Number.isFinite(latencyMs)) details.push(`${Math.round(latencyMs)}ms`);
    const lastUpdatedAt = station.lastUpdatedAt || selection.lastUpdatedAt || stationUsage.lastUpdatedAt || null;
    if (lastUpdatedAt) details.push(`ult. ${formatHour(lastUpdatedAt)}`);
    const apiStatus = station.apiStatus || selection.apiStatus || stationUsage.apiStatus || '';
    if (apiStatus) details.push(apiStatus);
    const failureReason = station.failureReason || selection.failureReason || stationUsage.failureReason || '';
    if (failureReason) details.push(`motivo: ${failureReason}`);

    return details.length
      ? `Base: ${stationLabel} (${details.join(', ')})`
      : `Base: ${stationLabel}`;
  }

  function applyClimateTheme(current) {
    const climateRoot = document.getElementById('clima');
    if (!climateRoot || !current) return;

    const themeClasses = [
      'weather-theme-initializing',
      'weather-theme-clear',
    ...
    climateRoot.classList.remove(...themeClasses);

    const weatherCode = Number(current.weatherCode);
    let nextTheme = 'weather-theme-clouds';

    if (!current.isDay) {
      nextTheme = 'weather-theme-night';
    } else if (weatherCode >= 95) {
      nextTheme = 'weather-theme-storm';
    } else if ((weatherCode >= 51 && weatherCode <= 67) || (weatherCode >= 80 && weatherCode <= 82)) {
      nextTheme = 'weather-theme-rain';
    } else if (weatherCode === 0 || weatherCode === 1) {
      nextTheme = 'weather-theme-clear';
    }

    climateRoot.classList.add(nextTheme);
  }
  ...
  function renderProviderStatusGrid(providerRuns) {
    ...
    dom.providerStatusGrid.innerHTML = visibleRuns
      .map((run) => {
        ...
        const stationText = buildStationPanelText(run.stationSelection);
        const messageText = stationText ? `${run.message} ${stationText}.` : run.message;
  ...
  function openReliabilityModal() {
    renderReliabilitySourcesModal();
    if (!dom.reliabilityModal) return;
    dom.reliabilityModal.hidden = false;
    dom.reliabilityModal.classList.add('is-open');
    dom.reliabilityModalClose?.focus();
  }

  function closeReliabilityModal() {
    if (!dom.reliabilityModal) return;
    dom.reliabilityModal.classList.remove('is-open');
    dom.reliabilityModal.hidden = true;
    dom.reliabilityDetailsBtn?.focus();
  }

  function getVisibleProviderCount(providerRuns) {
    return (Array.isArray(providerRuns) ? providerRuns : []).filter((run) => !run?.hidden && (run?.success || run?.status === 'partial')).length;
  }

  function renderReliabilitySourcesModal() {
    const bundle = state.weatherBundle;
    const visibleRuns = (Array.isArray(state.providerRuns) ? state.providerRuns : []).filter((run) => !run?.hidden);
    const activeRuns = visibleRuns.filter((run) => run?.success || run?.status === 'partial' || run?.status === 'loading');
    const confidence = Number.isFinite(bundle?.analytics?.confidence) ? `${bundle.analytics.confidence}%` : '--%';
    const consensus = bundle?.analytics?.sourceConsensus?.shortLabel || bundle?.modelAgreement?.consensusLevel || 'Em analise';

    if (dom.reliabilityModalSummary) {
      dom.reliabilityModalSummary.textContent = `${confidence} de confiabilidade. ${consensus}.`;
    }

    if (!dom.reliabilitySourcesList) return;

    const sourceCards = activeRuns.map((run) => {
      const statusLabel = run.status === 'loading'
        ? 'Sincronizando'
        : run.success
          ? (run.status === 'partial' ? 'Parcial' : 'Ativa')
          : 'Indisponivel';
      const weightText = Number.isFinite(run.weight) ? formatNumber(run.weight, 2) : '--';
      const latencyText = Number.isFinite(run.latencyMs) ? `${run.latencyMs}ms` : '--';
      const stationText = buildStationPanelText(run.stationSelection);
      const detail = stationText || run.message || 'Fonte usada na fusao meteorologica.';

      return `
        <article class="reliability-source-card ${run.success ? 'online' : 'partial'}">
          <div>
            <strong>${escapeModalHtml(run.label || run.providerKey || 'Fonte')}</strong>
            <span>${escapeModalHtml(statusLabel)}</span>
          </div>
          <p>${escapeModalHtml(detail)}</p>
          <dl>
            <dt>Peso</dt><dd>${escapeModalHtml(weightText)}</dd>
            <dt>Latencia</dt><dd>${escapeModalHtml(latencyText)}</dd>
          </dl>
        </article>
      `;
    });

    const radarCard = state.radar?.frames?.length ? `
      <article class="reliability-source-card online">
        <div><strong>Radar meteorologico</strong><span>Ativo</span></div>
        <p>Frames de radar usados como apoio visual para chuva e deslocamento.</p>
        <dl><dt>Frames</dt><dd>${state.radar.frames.length}</dd><dt>Tipo</dt><dd>Radar</dd></dl>
      </article>
    ` : '';

    const referenceCard = `
      <article class="reliability-source-card reference">
        <div><strong>Windy / Ventusky</strong><span>Referencia</span></div>
        <p>Apps de comparacao visual. Entram no calculo apenas quando houver dado/API ativo equivalente.</p>
        <dl><dt>Uso</dt><dd>Comparacao</dd><dt>Status</dt><dd>Opcional</dd></dl>
      </article>
    `;

    dom.reliabilitySourcesList.innerHTML = sourceCards.length
      ? `${sourceCards.join('')}${radarCard}${referenceCard}`
      : `
        <article class="reliability-source-card partial">
          <div><strong>Fontes meteorologicas</strong><span>Aguardando</span></div>
          <p>A primeira sincronizacao ainda nao terminou.</p>
          <dl><dt>Peso</dt><dd>--</dd><dt>Latencia</dt><dd>--</dd></dl>
        </article>
      `;
  }
  ...
  function renderClimateDetail() {
    ...
    dom.hourlyReference.textContent = `${selectedDay.label} • chuva, vento e tendencia para as proximas horas`;
  ...
  function renderCharts() {
    ...
    if (typeof Chart === 'undefined') {
      console.warn('[clima] Chart.js indisponivel. Graficos nao renderizados.');
      return;
    }
    ...
    const demoSeries = getProviderSeriesForChart(providerMap.get('openMeteo'), series);
    const trendSeries = series.map((entry) => ({
      temperature: entry.temperature,
      windSpeed: entry.windSpeed
    }));

    renderOverviewChart(series, demoSeries, trendSeries);
    renderWindChart(series, demoSeries, trendSeries);
    renderAtmosphereChart(series);
  }

  function renderOverviewChart(series, demoSeries, trendSeries) {
      ...
      data: {
        ...
        datasets: [
          ...
          },
          {
            type: 'line',
            label: 'Open-Meteo (°C)',
            data: demoSeries.map((entry) => entry.temperature),
            yAxisID: 'yTemp',
            borderColor: '#f59e0b',
            fill: false,
            tension: 0.25,
          ...
          {
            type: 'line',
            label: 'Tendencia atual (°C)',
            data: trendSeries.map((entry) => entry.temperature),
            yAxisID: 'yTemp',
            borderColor: '#8b5cf6',
            fill: false,
            tension: 0.25,
  ...
  function renderWindChart(series, demoSeries, trendSeries) {
    const labels = series.map((entry) => formatChartLabel(entry.time, state.chartRangeHours));
    ...
    climateCharts.wind = new Chart(dom.graficoWind.getContext('2d'), {
      data: {
        ...
        datasets: [
          ...
          {
            type: 'line',
            label: 'Open-Meteo vento (km/h)',
            data: demoSeries.map((entry) => entry.windSpeed),
            yAxisID: 'yWind',
            borderColor: '#fbbf24',
            fill: false,
            pointRadius: 0
          ...
          {
            type: 'line',
            label: 'Tendencia atual vento (km/h)',
            data: trendSeries.map((entry) => entry.windSpeed),
            yAxisID: 'yWind',
            borderColor: '#8b5cf6',
            fill: false,
            pointRadius: 0
  ...
  async function getCityWeather(cityName, options = {}) {
    const normalizedQuery = String(cityName || '').trim();
    if (normalizedQuery.length < 3) {
      renderSearchResults([]);
      throw createError('unsupported', 'Informe uma cidade com pelo menos 3 caracteres.');
    }
    ...
    if (options.showLoading) {
      showLoadingOverlay(true, 'Buscando cidade e bases proximas...');
    }
    setWeatherUpdateStatus(`Buscando "${normalizedQuery}"...`, 'updating');

    try {
      const location = await resolveCitySearchLocation(normalizedQuery);
      findNearestWorkingStation(location.lat, location.lon, {
        name: location.name
      }).catch(() => null);
      ...
      state.locationInfo = {
        message: `Cidade selecionada automaticamente: ${location.name}.`,
        tone: 'success'
      };
      state.selectedDayIndex = 0;
      state.animation.frameIndex = 0;
      ...
      const result = await getWeatherBundleForLocation(location, {
      ...
      if (requestId !== state.requestId) {
        return {
          location,
          ...result,
          station: result.bundle?.analytics?.stationUsage || null
        };
      }

      commitWeatherBundleResult(location, result, {
        statusMessage: 'Dados atualizados pela cidade pesquisada.',
        statusTone: 'success'
      });
      renderSearchResults([]);
      startAutoRefresh(Number(state.weatherBundle?.analytics?.confidence) < 70
        ? LOW_CONFIDENCE_RETRY_MS
        : getBaseRefreshDelayMs());
      queueEnhancementLoads();
      ...
      const restored = restoreLastGoodWeatherState(error);
      if (!restored) {
        setWeatherUpdateStatus('Erro na busca da cidade', 'error');
        if (dom.locationSearchStatus) {
          dom.locationSearchStatus.textContent = 'Nao foi possivel buscar esse local agora.';
        }
      }
      throw error;
    } finally {
  ...
  function commitWeatherBundleResult(location, result, options = {}) {
    state.location = {
      ...state.location,
      ...location
    };
    ...
    setWeatherUpdateStatus(
      state.cacheInfo.used
        ? (state.cacheInfo.saved || state.cacheInfo.stale ? 'Usando ultimo dado valido salvo' : 'Usando cache salvo')
        : (options.statusMessage || 'Dados atualizados'),
      state.cacheInfo.used ? 'cache' : (options.statusTone || 'success')
    );
  }

  async function resolveCitySearchLocation(query) {
    const normalizedQuery = String(query || '').trim();
    const cacheKey = `search:${normalizedQuery.toLowerCase()}`;
    const cached = readCache(cacheKey);
    const cacheAge = cached ? Date.now() - cached.timestamp : Number.POSITIVE_INFINITY;
    const results = cached && cacheAge <= CACHE_TTL.search
      ? cached.value
    ...
    const selected = pickBestSearchResult(normalizedQuery, results);
    if (!selected) {
      throw createError('unsupported', `Nenhum resultado para "${normalizedQuery}".`);
    }

    return {
      lat: Number(selected.lat),
      lon: Number(selected.lon),
      name: selected.label || normalizedQuery,
  ...
  function pickBestSearchResult(query, results) {
    const list = Array.isArray(results) ? results.filter((result) => (
    ...
    return list.find((result) => stripAccents(result.label || '').toLowerCase() === normalizedQuery)
      || list.find((result) => stripAccents(result.label || '').toLowerCase().startsWith(normalizedQuery))
      || list[0];
  }
  ...
  async function fetchSearchResults(query) {
    const coordinateMatch = parseCoordinateSearch(query);
    if (coordinateMatch) {
  ...
  function parseCoordinateSearch(query) {
    ...
    return {
      lat,
      lon,
      label: `Coordenadas ${lat.toFixed(4)}, ${lon.toFixed(4)}`,
      country: '',
      timezone: 'Busca direta por coordenadas'
    };
  }
  ...
  function initClimateMap() {
    if (typeof L === 'undefined' || !document.getElementById('map')) {
      console.warn('[clima] Leaflet ou container do mapa indisponivel.');
      return;
    }
    ...
    climateMarker.bindTooltip('Ponto de analise climatica', {
      direction: 'top',
      opacity: 0.92
    });
    ...
    window.VentoUXEnhancements?.attachClimateMap?.({
      map: climateMap,
      marker: climateMarker,
      getState: () => state,
      applyLocation
    });
  }
  ...
  function initDroneMap() {
    if (typeof L === 'undefined' || !document.getElementById('drone-map')) {
      console.warn('[drone] Leaflet ou container do mapa indisponivel.');
      return;
    }
    ...
    droneMarker.bindTooltip('Ponto operacional do drone', {
      direction: 'top',
      opacity: 0.92
    });
    ...
    window.VentoUXEnhancements?.attachDroneMap?.({
      map: droneMap,
      marker: droneMarker,
      radius: droneRadius,
      getState: () => state
  ...
  async function renderMapOverlays() {
    ...
    if (state.weatherLayer === 'drone' || state.weatherLayer === 'satellite') {
      clearRadarLayer();
      clearFieldOverlay();
      renderSpecialMapOverlay(state.weatherLayer);
      updateAnimationFrameLabel();
      restartAnimationLoop();
      return;
    }

    clearSpecialMapOverlay();
  ...
  function clearSpecialMapOverlay() {
    if (specialMapOverlay && climateMap?.hasLayer(specialMapOverlay)) {
      climateMap.removeLayer(specialMapOverlay);
    }
    specialMapOverlay = null;
    climateMap?.getContainer()?.classList.remove('map-layer-drone', 'map-layer-satellite');
  }

  function renderSpecialMapOverlay(layerKey) {
    if (!climateMap || typeof L === 'undefined') return;
    clearSpecialMapOverlay();

    const center = [state.location.lat, state.location.lon];
    ...
    climateMap.getContainer()?.classList.add(layerKey === 'drone' ? 'map-layer-drone' : 'map-layer-satellite');

    if (layerKey === 'satellite') {
      L.rectangle([[lat - 0.22, lon - 0.28], [lat + 0.22, lon + 0.28]], {
        color: '#38bdf8',
        weight: 1,
        opacity: 0.7,
        fillColor: '#0f766e',
      ...
      L.circle(center, {
        ...
        fillColor: '#22c55e',
        fillOpacity: 0.08
      }).bindTooltip('Visualização por satélite • nuvens, chuva, umidade e vegetação', { sticky: true }).addTo(group);
    } else {
      L.polygon(bounds, {
        color: '#38bdf8',
        weight: 2,
        ...
        fillColor: '#22c55e',
        fillOpacity: 0.16
      }).bindTooltip('Área selecionada para missão', { sticky: true }).addTo(group);
      L.polyline(route, {
        color: '#60a5fa',
        weight: 4,
        opacity: 0.9
      }).bindTooltip('Rota do drone', { sticky: true }).addTo(group);
      route.forEach((point, index) => {
        L.circleMarker(point, {
          radius: index === 0 ? 8 : 6,
          color: index === 0 ? '#f8fafc' : '#93c5fd',
          weight: 2,
          fillColor: index === 0 ? '#2563eb' : '#38bdf8',
          fillOpacity: 0.95
        }).bindTooltip(index === 0 ? 'Marcador inicial' : `Ponto de captura ${index}`, { sticky: true }).addTo(group);
      });
    }

    specialMapOverlay = group.addTo(climateMap);
    updateRadarStatus(layerKey === 'drone'
      ? 'Camada Drone ativa: área, rota e pontos de captura.'
      : 'Camada Satélite ativa: visualização orbital aplicada.');
  }
  ...
  function updateAnimationFrameLabel(frame) {
    if (!dom.animationFrameLabel) return;
  ...
  function buildFlightAnalysis(bundle) {
    const serviceAnalysis = window.SkyDroneServices?.flightSafetyService?.evaluateBundle?.(bundle, {
      cacheInfo: state.cacheInfo
    });
    if (serviceAnalysis) {
      return window.SkyDroneServices.flightSafetyService.toTabsAnalysis(
        serviceAnalysis,
        bundle.analytics?.sourceConsensus || null
      );
    }
    ...
    const badges = [];
    if (sourceConsensus?.windConflict) badges.push('Vento varia entre fontes');
    if (sourceConsensus?.rainConflict) badges.push('Chuva varia entre fontes');
  ...
  function updateStatusPanel(analysis) {
    ...
    if (analysis.overallStatus === 'safe') {
      ...
      if (analysis.smartRecommendation) {
        dom.statusSubtitle.textContent = analysis.smartRecommendation;
      }
  ...
  function updateRecommendations(analysis, current) {
    ...
    if (analysis.smartRecommendation) {
      recommendations.push({
        icon: analysis.overallStatus === 'danger' ? '!' : analysis.overallStatus === 'warning' ? '!' : 'OK',
        text: analysis.smartRecommendation
      });
    }
  ...
  function startAutoRefresh(delayMs = getBaseRefreshDelayMs()) {
    ...
    window.SkyDroneServices?.autoUpdateService?.stop?.();
    ...
    if (window.SkyDroneServices?.autoUpdateService?.schedule) {
      const handles = window.SkyDroneServices.autoUpdateService.schedule({
        ...
        onRefresh: () => {
          ...
          const confidence = Number(state.weatherBundle?.analytics?.confidence);
          const lowConfidence = Number.isFinite(confidence) && confidence < 70;
          refreshWeather({
            showLoading: false,
            force: lowConfidence || !state.resilience.online || state.resilience.consecutiveRefreshFailures > 0
          });
        }
      });
      state.timers.refresh = handles.refreshTimer;
      state.timers.countdown = handles.countdownTimer;
      return;
    }
  ...
  function friendlyProviderError(error, provider) {
    ...
    if (error.code === 'stale_station') {
      return `${provider.label}: estacao sem atualizacao recente; fallback automatico acionado.`;
    }
    if (error.code === 'invalid_station_data') {
      return `${provider.label}: leitura sem temperatura, vento ou umidade suficientes.`;
    }
    if (error.code === 'high_latency') {
      return `${provider.label}: latencia alta; peso reduzido e fallback automatico ativo.`;
    }
    if (error.code === 'controlled_offline') {
  ...
  function getServerProxyOrigins() {
    ...
    const serviceBackend = window.SkyDroneServices?.weatherService?.getConfiguredBackendOrigin?.() || '';
    if (serviceBackend) {
      origins.push(serviceBackend);
    }
    ...
    try {
      for (const attempt of attempts) {
        try {
          ...
          if (!response.ok) {
            const error = createError('provider_offline', `HTTP ${response.status}`);
  ...
  function readCache(key) {
    try {
      ...
      if (!parsed || !Number.isFinite(Number(parsed.timestamp))) {
  ...
  function isLikelyBrazilCoverage(location) {
